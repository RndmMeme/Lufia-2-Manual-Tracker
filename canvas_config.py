        ...
        for i, name in enumerate(characters_list):
            try:
                ...
                if image:
                    ...
                    maidens_canvas.tag_bind(image_id, "<Button-1>", lambda event, n=name, app=app: toggle_maiden_image(maidens_canvas, maiden_images, n, app))
...
def toggle_maiden_image(canvas, maiden_images, name, app):
    maiden_info = app.maiden_images.get(name)
    if maiden_info:
        
        if maiden_info['current_image'] == maiden_info['bw_image']:
            
            new_image = maiden_info['color_image']
            maiden_info['is_colored'] = True
        else:
           
            new_image = maiden_info['bw_image']
